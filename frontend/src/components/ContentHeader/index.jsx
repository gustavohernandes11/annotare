    ...
    return (
        <Styled.Header>
            ...
            <span>
                ...
                <p>{globalState.searchInputValue ? `Buscando por "${globalState.searchInputValue}"` : 'Suas anotações'}</p>
            </span>
            <span className='search-and-button-span'>
                <SearchInput id="search-input" onChange={(e) => globalActions.setSearchInputValue(e.target.value)} />
                <LargeButton primary="true" onClick={handleClick}>Adicionar</LargeButton>
                {
                    globalState.selectedCategory !== null
                    && (<IconButton onClick={() => setIsPopoverActive(true)} className="delete-category-icon">
